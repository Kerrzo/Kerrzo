; (function () {
  ...
  Rive({
      ...
      const req = new Request('42logo.riv');
      fetch(req).then((res) => {
          return res.arrayBuffer();
      }).then((buf) => {
          const asset = rive.load(new Uint8Array(buf)); // the Rive engine loads a Rive asset via raw bytes

          const artboard = asset.defaultArtboard();
          // now we can access the animations; let's get one called 'vibration'
          const revealAnim = artboard.animationByName('Reveal');
          const vibrationInstance = new rive.LinearAnimationInstance(revealAnim);
